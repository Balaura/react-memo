...
### Отображение счетчика в упрощенном режиме

В игре должен быть упрощенный режим, который включается с помощью чек-бокса на странице выбора уровня. В этом режиме игра завершается после трех ошибок, а не после первой, как в обычном режиме.
На игровом экране добавлен счетчик оставшихся попыток, если включен упрощенный режим.

Счетчик должен быть:

1. Понятным и информативным: сразу ясно, сколько ошибок можно еще допустить.
2. Позитивным: акцент на оставшихся попытках, а не на ошибках.
3. Простым: легко читается в процессе игры.
4. Адаптивным: подходит для локализации.
